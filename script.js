...
async function guardarSaldoEnBD() {
    ...
    await supabaseClient
        .from('perfiles')
        .upsert({ id: currentUser.id, saldo: credit });
}

// ==========================================
// 3. FUNCIONES DEL JUEGO Y APUESTAS
// ==========================================
...
// Apuestas: Ahora salta de manera dinámica para llegar rápido a $2000
...
async function executeSpin() {
    isSpinning = true;
    statusMessage.innerText = isFreeSpinsMode ? `GIRO DIVINO: ${freeSpinsLeft}` : (speedMult < 1 ? "GIRO TURBO" : "¡INVOCANDO LOS RAYOS!");
...
// SISTEMA DE PROBABILIDADES ENDURECIDO PARA QUE PAGUE MENOS
...
function getRandomSymbolWithProbability() {
    const weights = {
        'azul': 250, 'verde': 200, 'amarilla': 150, 'morada': 80, 'roja': 50,
        'caliz': 25, 'anillo': 15, 'reloj': 8, 'corona': 3,
        'zeus': doubleChance ? 10 : 4 
    };

    if (isFreeSpinsMode) {
        if (isSuperBonusMode) weights['orbe'] = 15; 
        else weights['orbe'] = 4; 
    } else {
        weights['orbe'] = 1; 
    }
    ...
    if (selectedId === 'orbe') {
        let multWeights;
        if (isSuperBonusMode) {
            multWeights = { 15: 1000, 25: 150, 50: 30, 100: 5, 250: 1, 500: 0.1 };
        } else {
            multWeights = { 2: 8000, 3: 3000, 5: 1000, 8: 200, 10: 80, 15: 30, 25: 10, 50: 2, 100: 0.5, 250: 0.1, 500: 0 };
        }
...
async function handleTumbles() {
    ...
    // CHECK STOP AUTOPLAY CONDITIONS
    if (autoSpinActive) {
...
// ==========================================
// 4. LÓGICA DE GIROS AUTOMÁTICOS (AUTOPLAY)
// ==========================================
...
document.querySelectorAll('.auto-count-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        speedMult = parseFloat(autoSpeedSelect.value);
...
async function startAutoPlay() {
    ...
    while (autoSpinActive && autoSpinsLeft > 0) {
        // Espera si está girando o en giros gratis
